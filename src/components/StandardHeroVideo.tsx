...
const StandardHeroVideo = ({
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const logoTopPosition = isMobile ? parseInt(logoTopPositionMobile, 10) : parseInt(logoTopPositionDesktop, 10);
  ...
  return <>
      {showLogo && <div ref={logoRef} className="absolute pointer-events-none transition-opacity duration-300 left-1/2 transform -translate-x-1/2 z-50" style={{
      ...
      <div className="w-full bg-black min-h-[calc(100vw*9/16)] relative overflow-hidden py-[18px]">
        ...
        overflow: 'hidden'
          ...
          <div className="absolute inset-0 z-10 bg-black transition-opacity duration-500" style={{
          opacity: placeholderOpacity
        }}>
            <ResponsiveImage src={fallbackImageUrl} alt="Video thumbnail" className="w-full h-full object-cover" onLoad={() => console.log('Fallback image loaded')} onError={() => {
            ...
            {!videoError && !videoLoaded && <div className="absolute inset-0 flex items-center justify-center">
          ...
          {/* Vimeo container */}
          <div ref={vimeoContainerRef} className="absolute top-0 left-0 w-full h-full" style={{
          width: '100%',
          height: '100%',
          transform: 'scale(1.3)',
      ...
      {(title || subtitle) && <div className="w-full bg-black py-10 md:py-16">
          <div className="max-w-3xl mx-auto text-center px-4">
            {title && <h2 className="text-white text-lg md:text-2xl font-swiss uppercase mb-2">
                {title}
              </h2>}
            {subtitle && <p className="text-white text-base md:text-2xl opacity-90 font-swiss drop-shadow-md px-2">
                {subtitle}
              </p>}
          </div>
        </div>}
